...
*  Copyright notice
*
*  (c) 2012-2015 Jonathan Heilmann <mail@jonathan-heilmann.de>
*  All rights reserved
*
 ...
 */
class tx_jhopengraphttnews_pi1 {
	...
	function extraItemMarkerProcessor($markerArray, $row, $conf, &$pObj) {
		...
		if($pObj->config['code'] == 'SINGLE') {
			...
			if($row['image'] != '') {
				...
				if(strpos($imagename , '.pdf')) {
					$picConf['file'] = 'uploads/pics/' . $imagename;
					$picConf['file.']['width'] = 403; // width for Facebook-pics
					$image = $GLOBALS['TSFE']->cObj->IMG_RESOURCE($picConf);
				} else {
